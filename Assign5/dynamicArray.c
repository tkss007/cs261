...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
    ...
    while (pIndex >= 0 && (*compare)(val, getDynArr(heap, pIndex)) == -1) {
        swapDynArr(heap, index, pIndex);
        index = pIndex;
        pIndex = (index - 1) / 2;
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
    ...
    assert(pos <= sizeDynArr(heap));
    int curIdx;
    int fstChildIdx = (2 * pos) + 1;
    int secChildIdx = (2 * pos) + 2;
    
    if (secChildIdx <= max) {
        curIdx = _smallerIndexHeap(heap, fstChildIdx, secChildIdx, compare);
        if ((*compare)(getDynArr(heap, pos), getDynArr(heap, curIdx)) == 1) {
            swapDynArr(heap, pos, curIdx);
            _adjustHeap(heap, max , curIdx, compare);
        }
    }
    else if (fstChildIdx <= max){
        if ((*compare)(getDynArr(heap, pos), getDynArr(heap, fstChildIdx)) == 1) {
            swapDynArr(heap, pos, fstChildIdx);
            _adjustHeap(heap, max, fstChildIdx, compare);
        }
    }
    /* FIXME: Write this */
}
...
{
    assert(heap != 0);
    for (int i = (sizeDynArr(heap) - 2) / 2; i >= 0 ; i--) {
